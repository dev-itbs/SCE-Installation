...
if [[ "$IS_ONPREM" == "true" ]]; then
    ...
    if [[ "$DOMAIN" == "localhost" || "$DOMAIN" == localhost:* ]]; then
        ...
        S3_PUBLIC_ENDPOINT="http://${DOMAIN}:9010"
    fi
...
STORAGE_PATH_BASE="$(slugify "$ECOSYSTEM_NAME")"
# S3 key prefixes are paths WITHIN the bucket — do NOT include the bucket name.
ask "S3 key prefix for uploads (no bucket name)" "uploads/";        S3_UPLOAD_PATH="$(printf "%s" "$REPLY" | tr '[:upper:]' '[:lower:]')"
ask "S3 key prefix for EMS incident files (no bucket name)" "incident/"; EMS_FILE_PATH="$(printf "%s" "$REPLY" | tr '[:upper:]' '[:lower:]')"
ask "S3 key prefix for final uploads (no bucket name)" "dest/";     UPLOAD_DEST_PATH="$(printf "%s" "$REPLY" | tr '[:upper:]' '[:lower:]')"

# ══════════════════════════════════════════════════════════════════════════════
title "STEP 6 — PHP App Settings (SCE-PHP-SYSTEMS)"
ask "PHP app host port (must be free on this machine)" "8880"; PHP_HOST_PORT="$REPLY"
PHP_PUBLIC_URL="$DOMAIN_URL"
[[ "$DOMAIN" == "localhost" ]] && PHP_PUBLIC_URL="http://localhost:${PHP_HOST_PORT}"
...
if [[ "$DOMAIN" == "localhost" || "$DOMAIN" == localhost:* ]]; then
    EASSIST_PLUGIN_DEFAULT="http://localhost:3002/dist-plugin/plugins.js"
else
    EASSIST_PLUGIN_DEFAULT="https://${DOMAIN}/dist-plugin/plugins.js"
fi
ask "EASSIST_PLUGIN (URL to eAssist plugins.js)" "${EASSIST_PLUGIN_DEFAULT}"; EASSIST_PLUGIN="$REPLY"
...
if [[ -n "$DIR_VAULT" ]]; then
    ...
    write_env "${DIR_VAULT}/postgres-wal.env" \
        "# PostgreSQL HA + WAL stack settings." \
        "# Host port 55432 avoids conflict with a local Windows PostgreSQL on 5432." \
        "POSTGRES_DB=${PG_DB}" \
        ...
        "" \
        "# Backup API admin GUI: http://localhost:8090/admin" \
        "# These seed the SQLite store on first boot. After that the GUI is the source of" \
        "# truth (Settings page); changing them here later has no effect unless the DB row" \
        "# is missing. Set a strong BACKUP_ADMIN_PASSWORD before first start." \
        "BACKUP_ADMIN_USERNAME=admin" \
        "BACKUP_ADMIN_PASSWORD=${BACKUP_PW}" \
        "BACKUP_API_DB_PATH=${BACKUP_DB_PATH}" \
        "BACKUP_SESSION_TTL_HOURS=24" \
        "# Set true only when serving the GUI over HTTPS." \
        ...
        "PGPOOL_EXTERNAL_PG_PRIMARY=" \
        "PGPOOL_EXTERNAL_PG_PRIMARY_PORT=" \
        "BACKUP_POSTGRES_PORT=5432" \
        "BACKUP_LOCAL_DIR=/backups" \
        "BACKUP_WAL_DIR=/wal-archive" \
        "" \
        "# S3-compatible backup storage. Set BACKUP_S3_ENABLED=true after filling these." \
        ...
        "" \
        "# Optional object-storage-to-object-storage backup." \
        "# This is separate from the database backup S3 settings above." \
        "OBJECT_BACKUP_ENABLED=false" \
        "OBJECT_BACKUP_INTERVAL_SECONDS=300" \
        "OBJECT_BACKUP_BATCH_SIZE=100" \
        "OBJECT_BACKUP_SOURCE_ENDPOINT=" \
        "OBJECT_BACKUP_SOURCE_REGION=us-east-1" \
        "OBJECT_BACKUP_SOURCE_BUCKET=" \
        "OBJECT_BACKUP_SOURCE_PREFIX=" \
        "OBJECT_BACKUP_SOURCE_ACCESS_KEY_ID=" \
        "OBJECT_BACKUP_SOURCE_SECRET_ACCESS_KEY=" \
        "OBJECT_BACKUP_SOURCE_FORCE_PATH_STYLE=true" \
        "OBJECT_BACKUP_TARGET_ENDPOINT=" \
        "OBJECT_BACKUP_TARGET_REGION=us-east-1" \
        "OBJECT_BACKUP_TARGET_BUCKET=" \
        "OBJECT_BACKUP_TARGET_PREFIX=" \
        "OBJECT_BACKUP_TARGET_ACCESS_KEY_ID=" \
        "OBJECT_BACKUP_TARGET_SECRET_ACCESS_KEY=" \
        "OBJECT_BACKUP_TARGET_FORCE_PATH_STYLE=true"
fi
...
if [[ -n "$DIR_PHP" ]]; then
    # Docker: PHP reaches MinIO by service name on internal port 9000.
    S3_PHP_ENDPOINT="$([[ "$IS_ONPREM" == "true" ]] && echo "http://sce-minio:9000" || echo "$S3_ENDPOINT")"
    write_env "${DIR_PHP}/.env" \
        "# Generated by SCE configure.sh" \
        "" \
        "# Host port mapping (change if 8880 is already in use)" \
        "PHP_HOST_PORT=${PHP_HOST_PORT}" \
        "" \
        "# Database" \
        ...
        "DB_NAME=${PG_DB}" \
        "DB_USER=postgres" \
        "DB_PASSWORD=${PG_SUPER_PW}" \
        "DB_SUPER_USER=postgres" \
        "DB_SUPER_PASSWORD=${PG_SUPER_PW}" \
        "" \
        "# Redis" \
        ...
        "# System identity" \
        "LGU=${LGU_CODE}" \
        "LGU_NAME=$(dotenv_quote "$LGU_NAME")" \
        "HASH_ID_SALT=${HASH_SALT}" \
        "PARTNER_ID=1" \
        "" \
        "# Upload paths" \
        "UPLOAD_TEMP_PATH=/var/www/sce/temp_uploads/" \
        "UPLOAD_TEMP_HREF_PATH=${PHP_PUBLIC_URL}/temp_uploads/" \
        "UPLOAD_DEST_PATH=${UPLOAD_DEST_PATH}" \
        "" \
        "# URLs" \
        "QRCODE_BASE_URL=$(dotenv_quote "${PHP_PUBLIC_URL}/CPA/#/deeplink?data=")" \
        "CPA_DOWNLOAD_URL=${PHP_PUBLIC_URL}/CPA/download" \
        "CPA_LOGIN_URL=$(dotenv_quote "${PHP_PUBLIC_URL}/CPA/#/login")" \
        "PYTHON_API_URL=http://sce-python-api:8000" \
        "SCE_SETTINGS_API_URL=${PHP_PUBLIC_URL}/UAC/api" \
        "" \
        "# Face / KYC" \
...
if [[ -n "$DIR_PYTHON" ]]; then
    # Docker: Python reaches MinIO by service name on internal port 9000.
    S3_PY_ENDPOINT="$([[ "$IS_ONPREM" == "true" ]] && echo "http://sce-minio:9000" || echo "$S3_ENDPOINT")"
    write_env "${DIR_PYTHON}/.env" \
        "# Generated by SCE configure.sh" \
        "" \
        "# Database" \
        "UAC_DB_URL=postgresql://postgres:${PG_SUPER_PW}@postgres-wal-pgpool:5432/${PG_DB}" \
        "IMPORT_DB_URL=postgresql://postgres:${PG_SUPER_PW}@postgres-wal-pgpool:5432/${PG_DB}" \
        "" \
        "# S3" \
